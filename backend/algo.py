...
def fitness_function(C, g, x, c):
    ...
    # Prevent negative values in square root
    if sqrt_arg < 0:
        return float('inf')
    
    ri1 = np.sqrt(sqrt_arg)
    d2i = a2 * ri1
...
def roulette_wheel_selection(population, total_delays, beta):
    # Handle NaN or infinite delays
    clean_delays = []
    for delay in total_delays:
        if np.isfinite(delay) and delay > 0:
            clean_delays.append(delay)
        else:
            clean_delays.append(1e6)  # Large penalty for invalid delays
    
    worst_delay = max(clean_delays)
    if worst_delay <= 0:
        worst_delay = 1.0
    
    probabilities = np.exp(-beta * np.array(clean_delays) / worst_delay)
